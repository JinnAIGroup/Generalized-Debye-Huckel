...
function g_PF = Activity(theta, BornR0, Rsh_c, Rsh_a, C1M, C3M, C4M, q1, q2, V0, V1, V2, V3, V4, diS, T)
  ...
  S1 = 1000*e/(4*pi*kBTe*epsln); S2 = 0.1*mol*e/(kBTe*epsln);  % PF3P64(7), PF3P80(13)
  
  C2M = q1*C1M;
  BornR_c = theta.*BornR0(1);
  ...
  LAMBDA = ( C1M/S2/1660.6 )*power(V1 - V2, 2)./LAMBDA;
      
  LDebye = sqrt( diS*epsln*kBTe*1.6606/e./(((1-LAMBDA)*q1*q1.*C1M - q1*q2*C1M)/S2) ); % PF Debye Length (3.5)
  ...
  lambda1 = (1 - sqrt(lam))./(2*power(Lcorr, 2));
  lambda2 = (1 + sqrt(lam))./(2*power(Lcorr, 2));
      
  d1 = lambda2.*( power(Lcorr, 2).*lambda1-1);
  d2 = lambda1.*( power(Lcorr, 2).*lambda2-1);
      
  THETA_c = (d1-d2)./( d1.*( sqrt(lambda1).*Rsh_c+1 ) - d2.*( sqrt(lambda2).*Rsh_c+1 ) );
  THETA_a = (d1-d2)./( d1.*( sqrt(lambda1).*Rsh_a+1 ) - d2.*( sqrt(lambda2).*Rsh_a+1 ) );
  
  gamma1 = exp( a1*(1./BornR_c - 1./BornR0(1) + (THETA_c-1)./Rsh_c) );  % cation activity
  gamma2 = exp( a2*(1./BornR_a - 1./BornR0(2) + (THETA_a-1)./Rsh_a) );  % anion activity
   
  g_PF = (abs(q2)*log(gamma1) + abs(q1)*log(gamma2))/(abs(q1) + abs(q2));  % mean activity in log
end