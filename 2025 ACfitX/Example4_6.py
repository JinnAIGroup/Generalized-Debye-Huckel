...
for salt in Salts:
  if salt == 'LaCl3':  # For Fig 7A
    ...
    for (T, T_i) in zip(Ts, range(len(Ts))):
      ...
      # Part 3: Example 4.5: Plot profiles around Na in NaCl + MgCl2 + H2O at C1M[-1] = 3 m MgCl2 + 3 m NaCl, T = 473.15
      ...
      print(" Pf.eltr[0], Pf.eltr_DH[0] =", np.around(Pf.eltr[0], 2), np.around(Pf.eltr_DH[0], 2))
  ...
  if salt == 'NaCl':  # For Fig 7B
    ...
    diCA = 5.92  # Young, KF, Frederikse, HPR (1973). Compilation of the static dielectric constant
    ...
    for (T, T_i) in zip(Ts, range(len(Ts))):
      ...
      if T_i == 0:  # Fig 6B at 298.15
        ...
        alpha_4 = 1 / (1 + 0.1 * IS)  # old
        ϵ_s_x_I_old = alpha_4 * ϵ_s_x + (1 - alpha_4) * diCA
        #print(' ϵ_s_x_I[-1], fac_pZ[-1]:', np.around(ϵ_s_x_I[-1], 2), np.around(fac_pZ[-1], 2))
        ...
        alpha_4 = 1 / (1 + 0.1 * IS)  # old
        ϵ_s_x_I_old = alpha_4 * ϵ_s_x + (1 - alpha_4) * diCA
        #print(' ϵ_s_x_I_old[-1], ϵ_s_x_I[-1], fac_pZ[-1]:', np.around(ϵ_s_x_I_old[-1], 2), np.around(ϵ_s_x_I[-1], 2), np.around(fac_pZ[-1], 2))

        eps_c1 = ϵ_s_x_I

        eps_a1 = ϵ_s_x - 16.2 * DF.C1m + 3.1 * DF.C1m ** 1.5  # ϵ_b=ϵ(m) Eq2,TableS.2[Sil23] at 573.15
        fc = eps_a1 / ϵ_s_x  # Eq2
        eps_a1 = fc * ewTsA[4]  # Fig 6B at 573.15, New1 Eq3[Sil23]
        sum1 = 0.01 * (C1M + C2M)  # Eq14[Sil23]
        sum2 = tMMB1 * C1M / (1 + 0.16 * C1M) + tMMB2 * C2M / (1 + 0.16 * C2M)
...
print(" Pf.eltr[0], Pf.eltr_2PF[0], Pf.eltr_PB[0] =", np.around(Pf.eltr[0], 2), np.around(Pf.eltr_2PF[0], 2), np.around(Pf.eltr_PB[0], 2))
print(" Pf.rho[26], Pf.rho_2PF[26], Pf.rho_PB[26], Ratio 1, 2 =", np.around(Pf.rho[26], 2), np.around(Pf.rho_2PF[26], 2), np.around(Pf.rho_PB[26], 2), np.around(Pf.rho_2PF[26]/Pf.rho[26], 2), np.around(Pf.rho_PB[26]/Pf.rho[26], 2))
...
#plt.plot(DF.C1m, eps_a1, 'bP',  label='$\epsilon_b$ at 300 $^{\circ}$C')
